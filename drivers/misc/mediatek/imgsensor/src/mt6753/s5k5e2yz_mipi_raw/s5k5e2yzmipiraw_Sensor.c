...
#include "s5k5e2yzmipiraw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "s5k5e2yz_camera_sensor"
#define LOG_1 LOG_INF("s5k5e2yz,MIPI 2LANE\n")
...
struct S5K5E2YZ_MIPI_otp_struct
{
...
#define RG_TYPICAL (0x342)
#define BG_TYPICAL (0x2C8)

static unsigned char get_otp_module_id(void)
{
	unsigned char module_id = 0;
	//printk("S5K5E2 OTP,%s +.\n",__func__);

	write_cmos_sensor(0x0a02, 0x02); //page set
	...
	printk("S5K5E2 ofilm++: Module ID = 0x%02x.\n",module_id);
	//printk("S5K5E2 OTP,%s -.\n",__func__);
	return module_id;
}

#define S5K5E2YZ_USE_OTP_FUNC
#if defined(S5K5E2YZ_USE_OTP_FUNC)

static kal_uint32 tRG_Ratio_typical = RG_TYPICAL;
static kal_uint32 tBG_Ratio_typical = BG_TYPICAL;
static struct S5K5E2YZ_MIPI_otp_struct current_otp;

static void S5K5E2YZ_MIPI_read_otp_wb(struct S5K5E2YZ_MIPI_otp_struct *otp)
{
	kal_uint32 R_to_G, B_to_G, G_to_G;
	kal_uint16 PageCount;
	for(PageCount = 3; PageCount>=2; PageCount--)
	{
		LOG_INF("[S5K5E2YZ]PageCount=%d\n", PageCount);	
		write_cmos_sensor(0x0a02, PageCount); //page set
		write_cmos_sensor(0x0a00, 0x01); //otp enable read
		if (read_cmos_sensor(0x0a43) == 0x1)
		{
			LOG_INF("[S5K5E2YZ]0x0a43 Valid.\n");	
			R_to_G = (read_cmos_sensor(0x0a2b)<<8)+read_cmos_sensor(0x0a2c);
			B_to_G = (read_cmos_sensor(0x0a2d)<<8)+read_cmos_sensor(0x0a2e);
			G_to_G = (read_cmos_sensor(0x0a2f)<<8)+read_cmos_sensor(0x0a30);
		}
		else if (read_cmos_sensor(0x0a23) == 0x1)
		{
			LOG_INF("[S5K5E2YZ]0x0a23 Valid.\n");	
			R_to_G = (read_cmos_sensor(0x0a0b)<<8)+read_cmos_sensor(0x0a0c);
			B_to_G = (read_cmos_sensor(0x0a0d)<<8)+read_cmos_sensor(0x0a0e);
			G_to_G = (read_cmos_sensor(0x0a0f)<<8)+read_cmos_sensor(0x0a10);
		}
		else
		{
			write_cmos_sensor(0x0a00, 0x00); //otp disable read
			LOG_INF("[S5K5E2YZ] [S5K5E2YZ_MIPI_read_otp_wb] Page[%d] invalid", PageCount);
			continue;
		}
		write_cmos_sensor(0x0a00, 0x00); //otp disable read
	...
	if(PageCount == 1)
	{
		LOG_INF("[S5K5E2YZ] [S5K5E2YZ_MIPI_read_otp_wb] otp all value is zero");
		return;
	}
	otp->R_to_G = (R_to_G * 0x400) / G_to_G;
	otp->B_to_G = (B_to_G * 0x400) / G_to_G;
	otp->G_to_G = G_to_G;
	printk("[S5K5E2YZ] Read from module: R_to_G=0x%x,B_to_G=0x%x,G_to_G=0x%x\n", R_to_G,B_to_G,G_to_G);	
	printk("[S5K5E2YZ] RG_Ratio_typical = 0x%x,RG_Ratio_typical = 0x%x.\n",tRG_Ratio_typical,tBG_Ratio_typical);
}

static void S5K5E2YZ_MIPI_algorithm_otp_wb1(struct S5K5E2YZ_MIPI_otp_struct *otp)
{
	kal_uint32 R_to_G, B_to_G, G_to_G;
	...
	printk("[S5K5E2YZ]  R_to_G=%d\n", R_to_G);	
	printk("[S5K5E2YZ]  B_to_G=%d\n", B_to_G);	
	printk("[S5K5E2YZ]  G_to_G=%d\n", G_to_G);
	...
	otp->G_Gain = G_Gain;
	otp->awb_status = 1;
	printk("[S5K5E2YZ] After calcuate: R_gain=0x%x\n", otp->R_Gain);	
	printk("[S5K5E2YZ] After calcuate: B_gain=0x%x\n", otp->B_Gain);	
	printk("[S5K5E2YZ] After calcuate: G_gain=0x%x\n", otp->G_Gain);
}



static void S5K5E2YZ_MIPI_write_otp_wb(struct S5K5E2YZ_MIPI_otp_struct *otp)
{
	...
	LOG_INF("[S5K5E2YZ]  R_GainH=0x%x\n", R_GainH);	
	LOG_INF("[S5K5E2YZ]  R_GainL=0x%x\n", R_GainL);	
	LOG_INF("[S5K5E2YZ]  B_GainH=0x%x\n", B_GainH);
	LOG_INF("[S5K5E2YZ]  B_GainL=0x%x\n", B_GainL);	
	LOG_INF("[S5K5E2YZ]  G_GainH=0x%x\n", G_GainH);	
	LOG_INF("[S5K5E2YZ]  G_GainL=0x%x\n", G_GainL);
	...
	LOG_INF("[S5K5E2YZ]final read:  [0x020e,0x%x]\n", read_cmos_sensor(0x020e));	
	LOG_INF("[S5K5E2YZ]final read:  [0x020f,0x%x]\n", read_cmos_sensor(0x020f));	
	LOG_INF("[S5K5E2YZ]final read:  [0x0210,0x%x]\n", read_cmos_sensor(0x0210));
	LOG_INF("[S5K5E2YZ]final read:  [0x0211,0x%x]\n", read_cmos_sensor(0x0211));	
	LOG_INF("[S5K5E2YZ]final read:  [0x0212,0x%x]\n", read_cmos_sensor(0x0212));	
	LOG_INF("[S5K5E2YZ]final read:  [0x0213,0x%x]\n", read_cmos_sensor(0x0213));
	LOG_INF("[S5K5E2YZ]final read:  [0x0214,0x%x]\n", read_cmos_sensor(0x0214));	
	LOG_INF("[S5K5E2YZ]final read:  [0x0215,0x%x]\n", read_cmos_sensor(0x0215));
	printk("%s,X.\n",__func__);
}
...
static void update_wb_register_from_otp(void)
{
	...
	if (current_otp.awb_status != 1)
	{
		...
		S5K5E2YZ_MIPI_read_otp_wb(&current_otp);
		S5K5E2YZ_MIPI_algorithm_otp_wb1(&current_otp);
	}

	S5K5E2YZ_MIPI_write_otp_wb(&current_otp);
    ...
    if((cmos_id == 0x5e20) && (otpMID == 0x07))
    {
        ...
        cam_id = mt_get_gpio_in(S5K5E2_TRULY_CAM_ID);
            ...
            if(0x01 == cam_id)
            {
                ...
                return S5K5E2YZ_SENSOR_ID;//0x5e22;
