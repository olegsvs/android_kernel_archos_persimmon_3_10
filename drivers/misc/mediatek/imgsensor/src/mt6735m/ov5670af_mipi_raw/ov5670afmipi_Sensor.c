...
int rg_h2,bg_h2,rg_bg_l2,g_ave2;
static int ov5670_read_otp(void)
{
	...
	{
		rg_h2=read_cmos_sensor(0x7028);
		bg_h2=read_cmos_sensor(0x7029);
		g_ave2=read_cmos_sensor(0x702A);
		rg_bg_l2=read_cmos_sensor(0x702B);
	}
	else if((awb_flag&0x30)==0x10)
	{
		rg_h2=read_cmos_sensor(0x7031);
		bg_h2=read_cmos_sensor(0x7032);
		g_ave2=read_cmos_sensor(0x7033);
		rg_bg_l2=read_cmos_sensor(0x7034);
	}	
	else if((awb_flag&0x0c)==0x04)
	{	
		rg_h2=read_cmos_sensor(0x703A);
		bg_h2=read_cmos_sensor(0x703B);
		g_ave2=read_cmos_sensor(0x703C);
		rg_bg_l2=read_cmos_sensor(0x703D);
	}
        printk("TCL :awb_flag=%x, %x ,%x ,%x ,%x\n",awb_flag,rg_h2,bg_h2,rg_bg_l2,g_ave2);
			...
			if (*sensor_id == imgsensor_info.sensor_id) {	
				...
				{			
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
                                	sprintf(Back_Camera_Name,"OV5670:E-WELLY:SUP1:5M_AF");  
					return ERROR_NONE;
				}
				else
				{
