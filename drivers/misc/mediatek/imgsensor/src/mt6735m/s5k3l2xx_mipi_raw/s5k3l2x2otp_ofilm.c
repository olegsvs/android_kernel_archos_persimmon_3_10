...
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0x70>>1)};	//0X6D
...
static int iCAM_i2c_Read(struct i2c_client *client, char *writebuf,
		    int writelen, char *readbuf, int readlen)
{
	...
	if(writelen!=0)
	{
		...
		else
		{
			...
			if((ret=i2c_master_send(client, (unsigned char *)(uintptr_t)I2CDMABuf_pa, writelen))!=writelen)
...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
   ...
   CAM_CALDB("[S5K3L2xxCAM_CAL] iWriteData\n" );
   ...
   CAM_CALDB("[S5K3L2xxCAM_CAL] iWriteData done\n" );
...
#endif
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            CAM_CALDB("[S5K3L2xxCAM_CAL] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALDB("[S5K3L2xxCAM_CAL] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    if(NULL == pWorkingBuff)
    {
     ...
     CAM_CALDB("[S5K3L2xxCAM_CAL] init Working buffer address 0x%p  command is 0x%8x\n", pWorkingBuff, (u32)a_u4Command);
    ...
    switch(a_u4Command)
    {
        case CAM_CALIOC_S_WRITE:    
            CAM_CALDB("[S5K3L2xxCAM_CAL] Write CMD \n");
        ...
        case CAM_CALIOC_G_READ:
            ...
            break;
        default :
      	     CAM_CALDB("[S5K3L2xxCAM_CAL] No CMD \n");
            i4RetValue = -EPERM;
        break;
    }
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        CAM_CALDB("[S5K3L2xxCAM_CAL] to user length %d \n", ptempbuf->u4Length);
        CAM_CALDB("[S5K3L2xxCAM_CAL] to user  Working buffer address 0x%p.\n", pWorkingBuff);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            CAM_CALDB("[S5K3L2xxCAM_CAL] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    CAM_CALDB("[S5K3L2xxCAM_CAL] CAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
		CAM_CALDB("[S5K3L2xxCAM_CAL] Opened, return -EBUSY\n");
        return -EBUSY;
    }
    else
    {
    ...
    {
        CAM_CALDB("[S5K3L2xxCAM_CAL] Fail to enable analog gain\n");
        return -EIO;
    }
...
static int CAM_CAL_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    {
        CAM_CALDB("[S5K3L2xxCAM_CAL] Fail to disable analog gain\n");
        return -EIO;
    }
    return 0;
}
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        CAM_CALDB("[S5K3L2xxCAM_CAL] Allocate device no failed\n");
    ...
    {
        CAM_CALDB("[S5K3L2xxCAM_CAL] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALDB("[S5K3L2xxCAM_CAL] Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALDB("[S5K3L2xxCAM_CAL] Attatch file operation failed\n");
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {             
int i4RetValue = 0;
    CAM_CALDB("[S5K3L2xxCAM_CAL] Attach I2C \n");
//    spin_lock_init(&g_CAM_CALLock);
    ...
    if(i4RetValue){
        CAM_CALDB("[S5K3L2xxCAM_CAL] register char device failed!\n");
        return i4RetValue;
    }


    CAM_CALDB("[S5K3L2xxCAM_CAL] Attached!! \n");
    return 0;                                                                                       
} 
...
static int CAM_CAL_i2c_remove(struct i2c_client *client)
{
	if(I2CDMABuf_va)
	{
		dma_free_coherent(&client->dev, 512, I2CDMABuf_va, I2CDMABuf_pa);
		I2CDMABuf_va = NULL;
		I2CDMABuf_pa = 0;
	}
    return 0;
...
static int __init CAM_CAL_i2C_init_OFILM(void)
{
    i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALDB("failed to register S5K3L2xxCAM_CAL driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALDB("failed to register S5K3L2xxCAM_CAL driver, 2nd time\n");
        return -ENODEV;
    }	
...
static void __exit CAM_CAL_i2C_exit_OFILM(void)
{
	platform_driver_unregister(&g_stCAM_CAL_Driver);
}

module_init(CAM_CAL_i2C_init_OFILM);
module_exit(CAM_CAL_i2C_exit_OFILM);
