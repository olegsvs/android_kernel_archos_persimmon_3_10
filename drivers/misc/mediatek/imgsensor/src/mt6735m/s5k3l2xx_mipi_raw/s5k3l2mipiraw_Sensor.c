...
static inline void S5K3L2XX_write_cmos_sensor1(u16 addr, u32 para)
{
	char puSendCmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) ,  (char)(para >> 8),	(char)(para & 0xFF) };
	iWriteRegI2C(puSendCmd , 4, imgsensor.i2c_write_id);
}
...
static inline kal_uint16 S5K3L2XX_read_cmos_sensor1(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
#define S5K3L2XX_GAIN_DEFAULT       0x0100
static struct S5K3L2XX_MIPI_otp_struct current_otp;

static void S5K3L2XX_MIPI_read_otp_wb(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   ...
   LOG_INF("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb]Flag 0x00=0x%x\n", FLG);
  	...
  	if(FLG==0)
   	...
   	else
   	{
		...
		current_R = S5K3L2XX_read_cmos_sensor1(0x0012);
  		...
  		LOG_INF("[S5K3L2XX] [S5K3L2XX_MIPI_read_otp_wb]Read from Module: current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
...
static void S5K3L2XX_MIPI_algorithm_otp_wb1(struct S5K3L2XX_MIPI_otp_struct *otp)
{
	...
	if(r_ratio >= 512 )
   {
    	if(b_ratio >= 512) 
        {
        	R_GAIN = S5K3L2XX_GAIN_DEFAULT * r_ratio / 512;
            G_GAIN = S5K3L2XX_GAIN_DEFAULT;     
            B_GAIN = S5K3L2XX_GAIN_DEFAULT * b_ratio / 512;
   		}
        else
        {
           	R_GAIN = S5K3L2XX_GAIN_DEFAULT * 512 / b_ratio  * r_ratio / 512;
           	G_GAIN = S5K3L2XX_GAIN_DEFAULT * 512 / b_ratio;
           	B_GAIN = S5K3L2XX_GAIN_DEFAULT;    
        }
   }
	else                      
   {
   		if(b_ratio >= 512)
    	{
      		R_GAIN = S5K3L2XX_GAIN_DEFAULT;    
      	  	G_GAIN = S5K3L2XX_GAIN_DEFAULT * 512/ r_ratio ;
           	B_GAIN = S5K3L2XX_GAIN_DEFAULT * 512 / r_ratio * b_ratio / 512;
        } 
        else 
        {
           	Gr_GAIN = S5K3L2XX_GAIN_DEFAULT * 512 / r_ratio;
           	Gb_GAIN = S5K3L2XX_GAIN_DEFAULT * 512 / b_ratio;
   ...
   printk("[S5K3L2XX] [S5K3L2XX_MIPI_algorithm_otp_wb1]After calcuate: R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);    
}



static void S5K3L2XX_MIPI_write_otp_wb(struct S5K3L2XX_MIPI_otp_struct *otp)
{
   ...
   S5K3L2XX_write_cmos_sensor1(0x0214, G_GAIN);
	...
	printk("%s,[S5K3L2xx]final read:  [0x020e,0x%x]\n",__func__,read_cmos_sensor(0x020e));	
	printk("%s,[S5K3L2xx]final read:  [0x020f,0x%x]\n", __func__,read_cmos_sensor(0x020f));	
	printk("%s,[S5K3L2xx]final read:  [0x0210,0x%x]\n", __func__,read_cmos_sensor(0x0210));
	printk("%s,[S5K3L2xx]final read:  [0x0211,0x%x]\n", __func__,read_cmos_sensor(0x0211));	
	printk("%s,[S5K3L2xx]final read:  [0x0212,0x%x]\n", __func__,read_cmos_sensor(0x0212));	
	printk("%s,[S5K3L2xx]final read:  [0x0213,0x%x]\n", __func__,read_cmos_sensor(0x0213));
	printk("%s,[S5K3L2xx]final read:  [0x0214,0x%x]\n", __func__,read_cmos_sensor(0x0214));	
	printk("%s,[S5K3L2xx]final read:  [0x0215,0x%x]\n", __func__,read_cmos_sensor(0x0215));
}



static void S5K3L2XX_MIPI_get_calc_wb_data_from_otp(void)
{
   printk("[S5K3L2XX] %s, +\n", __func__);  
...
static void S5K3L2XX_MIPI_update_wb_register_from_otp(void)
{  
   printk("[S5K3L2XX] %s, +\n", __func__);  
...
static kal_uint32 return_sensor_id()
{
	...
	//	mdelay(100);
	//}
    kal_uint16 otpMID = S5K3L2XX_read_cmos_sensor1(0x000b);
#if 0
    kal_uint16 otpValue = 0;
    kal_uint32 i4for = 0;
    for(;i4for <16;i4for++)
      {
           otpValue = S5K3L2XX_read_cmos_sensor1(i4for);
	   printk("[XX return id]OTP info -- %d = 0x%4x\n",i4for,otpValue);
      }
#endif
    if(otpMID == 0x0007)
      {
...
static void slim_video_setting()
{
	...
	write_cmos_sensor(0x6F12,0x40); //10                                   
...
static kal_uint32 open(void)
{
    ...
    for(;i4for <16;i4for++)
      {
           otpValue = S5K3L2XX_read_cmos_sensor1(i4for);
	   printk("[XX open]OTP info -- %d = 0x%4x\n",i4for,otpValue);
      }
#endif
...
static kal_uint32 close(void)
{
    ...
    for(;i <16;i++)
      {
           otpValue = S5K3L2XX_read_cmos_sensor1(i);
	   printk("[XX close]OTP info -- %d = 0x%4x\n",i,otpValue);
      }
#endif
