...
#include "s5k3l2yymipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L2YY_camera_sensor"
#define LOG_1 LOG_INF("S5K3L2YY,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5K3L2YY_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
struct S5K3L2YY_MIPI_otp_struct
{
...
#define S5K3L2YYOTP_WRITE_ID         0xA0
static inline void S5K3L2YY_write_cmos_sensor1(u16 addr, u32 para)
{
	char puSendCmd[4] = {(char)(addr >> 8) , (char)(addr & 0xFF) ,  (char)(para >> 8),	(char)(para & 0xFF) };
	iWriteRegI2C(puSendCmd , 4, imgsensor.i2c_write_id);
}
...
static inline kal_uint16 S5K3L2YY_read_cmos_sensor1(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	
	char puSendCmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
	iReadRegI2C(puSendCmd , 2, (u8*)&get_byte, 1, S5K3L2YYOTP_WRITE_ID);
	return get_byte&0x00ff;
}

#define S5K3L2YY_USE_AWB_OTP

#if defined(S5K3L2YY_USE_AWB_OTP)

#define S5K3L2YY_GAIN_DEFAULT       0x0100
static struct S5K3L2YY_MIPI_otp_struct current_otp;

static void S5K3L2YY_MIPI_read_otp_wb(struct S5K3L2YY_MIPI_otp_struct *otp)
{
   	...
   	FLG = S5K3L2YY_read_cmos_sensor1(0x0000); 
   LOG_INF("[S5K3L2YY] [S5K3L2YY_MIPI_read_otp_wb]Flag 0x00=0x%x\n", FLG);
   printk("[S5K3L2YY] [S5K3L2YY_MIPI_read_otp_wb]Flag 0x00=0x%x\n", FLG);
  	if(FLG==0)
   	...
   	else
   	{
		...
		current_R = S5K3L2YY_read_cmos_sensor1(0x0012);
  		current_Gr = S5K3L2YY_read_cmos_sensor1(0x0013);
  		current_Gb = S5K3L2YY_read_cmos_sensor1(0x0014);
  		current_B = S5K3L2YY_read_cmos_sensor1(0x0015); 
		golden_R = S5K3L2YY_read_cmos_sensor1(0x0016); 
		golden_Gr = S5K3L2YY_read_cmos_sensor1(0x0017); 
		golden_Gb = S5K3L2YY_read_cmos_sensor1(0x0018); 
		golden_B = S5K3L2YY_read_cmos_sensor1(0x0019); 

  		LOG_INF("[S5K3L2YY] [S5K3L2YY_MIPI_read_otp_wb]Read from Module: current_R=0x%x, current_Gr=0x%x, current_Gb=0x%x, current_B=0x%x\n", current_R, current_Gr, current_Gb, current_B);
  		LOG_INF("[S5K3L2YY] [S5K3L2YY_MIPI_read_otp_wb]Read from Module: golden_R=0x%x, golden_Gr=0x%x, golden_Gb=0x%x, golden_B=0x%x\n", golden_R, golden_Gr, golden_Gb, golden_B);
   		...
   		printk("[S5K3L2YY] [S5K3L2YY_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", otp->r_ratio, otp->b_ratio);      
   	}
}



static void S5K3L2YY_MIPI_algorithm_otp_wb1(struct S5K3L2YY_MIPI_otp_struct *otp)
{
	...
	if(r_ratio >= 512 )
   {
    	if(b_ratio >= 512) 
        {
        	R_GAIN = S5K3L2YY_GAIN_DEFAULT * r_ratio / 512;
            G_GAIN = S5K3L2YY_GAIN_DEFAULT;     
            B_GAIN = S5K3L2YY_GAIN_DEFAULT * b_ratio / 512;
   		}
        else
        {
           	R_GAIN = S5K3L2YY_GAIN_DEFAULT * 512 / b_ratio  * r_ratio / 512;
           	G_GAIN = S5K3L2YY_GAIN_DEFAULT * 512 / b_ratio;
           	B_GAIN = S5K3L2YY_GAIN_DEFAULT;    
        }
   }
	else                      
   {
   		if(b_ratio >= 512)
    	{
      		R_GAIN = S5K3L2YY_GAIN_DEFAULT;    
      	  	G_GAIN = S5K3L2YY_GAIN_DEFAULT * 512/ r_ratio ;
           	B_GAIN = S5K3L2YY_GAIN_DEFAULT * 512 / r_ratio * b_ratio / 512;
        } 
        else 
        {
           	Gr_GAIN = S5K3L2YY_GAIN_DEFAULT * 512 / r_ratio;
           	Gb_GAIN = S5K3L2YY_GAIN_DEFAULT * 512 / b_ratio;
   ...
   printk("[S5K3L2YY] [S5K3L2YY_MIPI_algorithm_otp_wb1]After calcuate: R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", otp->R_Gain, otp->B_Gain, otp->G_Gain);    
}



static void S5K3L2YY_MIPI_write_otp_wb(struct S5K3L2YY_MIPI_otp_struct *otp)
{
   ...
   S5K3L2YY_write_cmos_sensor1(0x020e, G_GAIN);
   S5K3L2YY_write_cmos_sensor1(0x0210, R_GAIN);
   S5K3L2YY_write_cmos_sensor1(0x0212, B_GAIN);
   S5K3L2YY_write_cmos_sensor1(0x0214, G_GAIN);

	

	printk("%s,[S5K3L2yy]final read:  [0x020e,0x%x]\n",__func__,read_cmos_sensor(0x020e));	
	printk("%s,[S5K3L2yy]final read:  [0x020f,0x%x]\n", __func__,read_cmos_sensor(0x020f));	
	printk("%s,[S5K3L2yy]final read:  [0x0210,0x%x]\n", __func__,read_cmos_sensor(0x0210));
	printk("%s,[S5K3L2yy]final read:  [0x0211,0x%x]\n", __func__,read_cmos_sensor(0x0211));	
	printk("%s,[S5K3L2yy]final read:  [0x0212,0x%x]\n", __func__,read_cmos_sensor(0x0212));	
	printk("%s,[S5K3L2yy]final read:  [0x0213,0x%x]\n", __func__,read_cmos_sensor(0x0213));
	printk("%s,[S5K3L2yy]final read:  [0x0214,0x%x]\n", __func__,read_cmos_sensor(0x0214));	
	printk("%s,[S5K3L2yy]final read:  [0x0215,0x%x]\n", __func__,read_cmos_sensor(0x0215));
}



static void S5K3L2YY_MIPI_get_calc_wb_data_from_otp(void)
{
   printk("[S5K3L2YY] %s, +\n", __func__);  
   S5K3L2YY_MIPI_read_otp_wb(&current_otp);
   S5K3L2YY_MIPI_algorithm_otp_wb1(&current_otp);
   printk("[S5K3L2YY] %s, -\n", __func__);  
}

static void S5K3L2YY_MIPI_update_wb_register_from_otp(void)
{  
   printk("[S5K3L2YY] %s, +\n", __func__);  
   S5K3L2YY_MIPI_write_otp_wb(&current_otp);
   printk("[S5K3L2YY] %s, -\n", __func__);  
}
...
static kal_uint32 return_sensor_id()
{
    ...
    kal_uint16 otpMID = S5K3L2YY_read_cmos_sensor1(0x0004);
    kal_uint16 cmos_id = ((read_cmos_sensor(0x0000)<<8)|read_cmos_sensor(0x0001));
#if 0
    kal_uint16 otpValue = 0;
    kal_uint32 i4for = 0;
    for(;i4for <16;i4for++)
      {
           otpValue = S5K3L2YY_read_cmos_sensor1(i4for);
	   printk("[YY return id]OTP info -- %d = 0x%4x\n",i4for,otpValue);
      }
#endif
    if((cmos_id == 0x30c2) && (otpMID == 0x0002))
      {
	LOG_INF(" Main camera search stage , detected.\n");
	return S5K3L2YY_SENSOR_ID;
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0101, 0x00);  //mirror 0x03
  write_cmos_sensor1(0x6028, 0xD000);
...
{	
	...
	write_cmos_sensor1(0x3010, 0x0000);		//first Pixel:B
...
*
*************************************************************************/
static int s5k3l2yy_sysfs_init(void);
...
//#ifdef S5K3L2YY_USE_AWB_OTP
#if 0
	S5K3L2YY_MIPI_get_calc_wb_data_from_otp();
#endif
                return ERROR_NONE;
            }
...
static const char *s5k3l2yyVendor = "SAMSUNG";
static const char *s5k3l2yyNAME = "s5k3l2yy_main";
static const char *s5k3l2yySize = "13.0M";
				  ...
				  struct device_attribute *attr, char *buf)
{
  ...
  sprintf(buf, "%s %s %s\n", s5k3l2yyVendor, s5k3l2yyNAME, s5k3l2yySize);
  ret = strlen(buf) + 1;
...
static struct kobject *android_s5k3l2yy = NULL;

static int s5k3l2yy_sysfs_init(void)
{
  int ret ;
  LOG_INF("kobject creat and add\n");
  if(android_s5k3l2yy == NULL){
    android_s5k3l2yy = kobject_create_and_add("android_camera", NULL);
    if (android_s5k3l2yy == NULL) {
    ...
    ret = sysfs_create_file(android_s5k3l2yy, &dev_attr_sensor.attr);
    if (ret) {
      LOG_INF("sysfs_create_file " \
	      "failed\n");
      kobject_del(android_s5k3l2yy);
      android_s5k3l2yy = NULL;
    }
  }
  return 0 ;
}
...
static kal_uint32 open(void)
{
    ...
    for(;i4for <16;i4for++)
      {
           otpValue = S5K3L2YY_read_cmos_sensor1(i4for);
	   printk("[YY open]OTP info -- %d = 0x%4x\n",i4for,otpValue);
      }
#endif
    LOG_1;
    LOG_2;
...
static kal_uint32 close(void)
{
    ...
    for(;i4for <16;i4for++)
      {
           otpValue = S5K3L2YY_read_cmos_sensor1(i4for);
	   printk("[YY close]OTP info -- %d = 0x%4x\n",i4for,otpValue);
      }
#endif
    LOG_INF("E\n");
                      ...
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    spin_unlock(&imgsensor_drv_lock);
#if defined(S5K3L2YY_USE_AWB_OTP)
	S5K3L2YY_MIPI_update_wb_register_from_otp();
#endif
    preview_setting();
